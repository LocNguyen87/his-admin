...
var kenny = function() {
  ...
  $(document).on("click", "#add_todo", function(e) {
    if (!$('.new-todo input').val().length == 0) {
      $(
        ...
        random + '"/><label for="checkbox' + random + '">' + $(
          '.new-todo input').val() + '</label></div></li>').insertAfter(
  ...
  $("#input_msg_send").keypress(function(e) {
    if ((e.which == 13) && (!$(this).val().length == 0)) {
      $(
        '<li class="self"><div class="self-msg-wrap"><div class="msg block pull-right">' +
        $(this).val() +
        '<div class="msg-per-detail mt-5"><span class="msg-time txt-grey">4:30pm</span></div></div></div><div class="clearfix"></div></li>'
      ).insertAfter(".chat-content > ul li:last-child");
      $(this).val('');
    } else if (e.which == 13) {
